...
var board = new firmata.Board("/dev/ttyACM0", function(){// ACM (Abstract Control Model)
    ...
    board.pinMode(13, board.MODES.OUTPUT); // Posamezni pin konfiguriramo, da deluje kot vhod ali izhod, deluje tudi brez konfiguracije
});
...
function handler(req, res) { // "handler", ki je uporabljen pri require("http").createServer(handler)
    ...
    function (err, data) {                    // klientu datoteko primer06.html iz diska strežnika
        if (err) {
