...
var board = new firmata.Board("/dev/ttyACM0", function(){// ACM (Abstract Control Model)
    ...
    board.pinMode(13, board.MODES.OUTPUT); // Posamezni pin konfiguriramo, da deluje kot vhod ali izhod ,dela tudi brez konfiguracije
});
...
io.sockets.on("connection", function(socket) {
    socket.on("ukazArduinu", function(štUkaza) {
        ...
        if (štUkaza == "2") {
            board.digitalWrite(12, board.HIGH); // zapišemo +5V na pin8
        }
        if (štUkaza == "3") {
            board.digitalWrite(12, board.LOW); // zapišemo 0V na pin8
        }
        if (štUkaza == "4") {
            board.digitalWrite(8, board.HIGH); // zapišemo 0V na pin8
        }
        if (štUkaza == "5") {
            board.digitalWrite(8, board.LOW); // zapišemo 0V na pin8
        }
        if (štUkaza == "6") {
            board.digitalWrite(7, board.HIGH); // zapišemo 0V na pin8
        }
        if (štUkaza == "7") {
            board.digitalWrite(7, board.LOW); // zapišemo 0V na pin8
        }
        if (štUkaza == "8") {
            board.digitalWrite(13, board.HIGH); // zapišemo 0V na pin8
            board.digitalWrite(12, board.HIGH); // zapišemo 0V na pin8
            board.digitalWrite(8, board.HIGH); // zapišemo 0V na pin8
            board.digitalWrite(7, board.HIGH); // zapišemo 0V na pin8
        }
        if (štUkaza == "9") {
            board.digitalWrite(13, board.LOW); // zapišemo 0V na pin8
            board.digitalWrite(12, board.LOW); // zapišemo 0V na pin8
            board.digitalWrite(8, board.LOW); // zapišemo 0V na pin8
            board.digitalWrite(7, board.LOW); // zapišemo 0V na pin8
        }
